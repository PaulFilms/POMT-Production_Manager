...
def tbl_bunits(df: pd.DataFrame) -> int:
    ...
    if filter_str and filter_str != '':
        mask = df_filter1.select_dtypes(include=['object']).apply(
                lambda col: col.str.contains(filter_str, na=False)
            ).any(axis=1)
        df_filter2 = df_filter1[mask]
    else: 
...
#         DB.select(f"SELECT * FROM pedidos WHERE bu_id='{bu_id}'"),
...
#             fecha_ini = datetime.fromtimestamp(fecha_ini)
#             fecha_fin = datetime.fromtimestamp(fecha_fin)
#             time_dict = {
...
#                 'text': {'headline': fila['id'], 'text': fila['info']},
#             }
...
#             timeline(time_data)
        ...
        #t.container(border=False, height=10) s
    ...
    #     st.dataframe(
    #         df_pedidos[['id', '#', 'info', 'contraseña', 'usuario_id', 'fecha_ini', 'fecha_fin',]],
